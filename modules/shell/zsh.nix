{ config, ... }:
{
  ...
  flake.nixosModules.zsh =
    ...
    let
      inherit (lib) mkIf attrValues any;
    in
    {
      # Enable zsh system wide if any user has zsh module enabled
      # Required to be set as user shell
      programs.zsh.enable = any (value: value.modules.zsh.enable) (attrValues config.home-manager.users);
