  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.zsh = {
      ...
      initContent = ''
        ...
        NEWLINE=$'\n'
        PROMPT="''${NEWLINE}\$(export retval=\$?)%{$(tput setaf 15 bold)%}%D{%a %d %b %Y %T} %{$(tput setaf 2)%}%M%{$(tput sgr0)%} [%!]%{%F{green}%}\$(git_branch)%{%F{none}%}\$([[ \$retval -ne 0 ]] && echo \" Exited with \$retval.\")''${NEWLINE}%{$(tput setaf 2 bold)%}%n@%m %{$(tput setaf 4)%}%~%{$(tput sgr0)%} %#$([[ $SHLVL -ne 1 ]] && echo "$SHLVL")> "
      '';
